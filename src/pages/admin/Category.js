...
export default function Category() {
  ...
  const handleFetchItem = async (id) => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/news/category/${id}`);
      setItem(res.data)
      onOpenModal();
      setLoading(false);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      const res = await axios.delete(`/api/news/category/${id}`);
      fetchCategoryList();
  ...
  const handleSubmit = async (item) => {
    if (item?.id) {
      try {
        const res = await axios.put(`/api/news/category/${item.id}`, item);
        fetchCategoryList();
        message.success(res.data.message)
        onCloseModal();
      } catch (err) {
      ...
      try {
        const res = await axios.post(`/api/news/category`, item);
        fetchCategoryList();
        message.success(res.data.message)
        onCloseModal();
      } catch (err) {
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <BreadCrumd title='Danh mục' subtitle='Danh sách' />
